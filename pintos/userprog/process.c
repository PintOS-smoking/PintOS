...
static bool lazy_load_segment(struct page *page, void *aux)
{
    ...
    /* * [수정됨]
     * file_seek + file_read 대신 file_read_at을 사용합니다.
     * file_read_at(file, buffer, size, file_ofs)
     */
    if (file_read_at(info->file, page->frame->kva, info->page_read_bytes, info->ofs) !=
        (int)info->page_read_bytes)
    {
        // palloc_free_page(page->frame->kva);
        // vm_do_claim_page 또는 vm_claim_page이 담당해서 정리해주므로, 신경 쓰지 않아도 된다.
    ...
    /* * 로딩이 완료되었으므로 파일과 info 구조체를 정리합니다.
     * (Pintos 구현에 따라 파일은 계속 열어둬야 하는 경우도 있지만,
     * 일반적인 lazy loading 구현에서는 여기서 닫아도 무방하거나,
     * mmap 등의 구현 여부에 따라 달라질 수 있습니다.
     * 작성하신 기존 로직을 존중하여 close를 유지합니다.)
     */
    file_close(info->file);
    free(info);
                         ...
                         uint32_t zero_bytes, bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0)
    {
        ...
        {
            file_close(aux->file); // reopen한 파일 닫기
            free(aux);
            return false;
        }
